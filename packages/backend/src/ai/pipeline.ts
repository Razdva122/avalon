...
export function decisionInstructions(request: BotRequest) {
  ...
  return (
    ...
    (['selectTeam', 'votingForTeam'].includes(action)
      ? ' Voting patterns: compare individual votes across BOTH rejectedProposals and approvedProposals, ordered by mission/attempt. Look for repeated groups supporting the same seats even when those voters are off-team, and jointly blocking alternatives. After a completed mission fails or reliable evidence identifies Evil, revisit who promoted that roster and who opposed safer alternatives. Explain the strongest pattern using exact mission/attempt and seats; retain a changed hypothesis in evidence when relevant. Forced proposals contain NO voluntary votes. Shared votes are suspicion, NEVER proof: Good may agree independently, prefer themselves, or avoid a dangerous fifth leader; Evil may reject allies to disguise cooperation. Compare these explanations and contrary votes. Good: if several suspects approve your roster, reconsider which member benefits them and compare a safer roster; do not automatically reject or override reliable private knowledge. Self-inclusion is a preference, not a reason to ignore a stronger evidence-backed alternative, including a repeated voting pattern. Evil: consider what your votes reveal about allies, while prioritizing victory over concealment.'
      : '') +
